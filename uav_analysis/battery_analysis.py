...
def battery_analyzer_sympy(batt_name="Tattu 22Ah Li", ):
    ...
    voltage_request = symbols("voltage_request")
    base_voltage = symbols("base_voltage")
    module_volume = symbols("module_volume")
    volume_percent = symbols("volume_percent")
    chord_1 = symbols("chord_1")
    chord_2 = symbols("chord_2")
    thickness = symbols("thickness")
    span = symbols("span")

    series_count = ceiling(voltage_request / base_voltage)
    ...
    p_count = floor(goal_volume / min_pack_volume)
...
def battery_analyzer(batt_name="Tattu 22Ah Li", ):
    ...
    thickness = 12.0  # where dows it come from?
    ...
    c_continuous = float(BATTERIES[batt_name]["Cont. Discharge Rate [C]"])  # not used
